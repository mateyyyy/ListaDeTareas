import React, { useState } from 'react'
import styles from './InfoDisplay.module.scss'
import Card from '../Card'
import Edit from '../Edit';
import DeleteButton from '../DeleteButton';

export default function InfoDisplay({element, url, updateState, blur}) {

  const [allContent, setAllContent] = useState(false);

  return (
    <div id={styles.PrinDiv} className={blur? styles.setBlur: null}>
      
      <div id={styles.secDiv}>
      <button id={styles.buton} onClick={() => (setAllContent(!allContent))}>
            <h3>Nombre : {element.name.slice(0, 20)}</h3>
            {element.description ?
              <p>Descripcion : {element.description.slice(0,25)}</p>
              :
              (null)}

              {element.created!=null?
              <p>Creado : {element.created.split('T')[0]}</p>
              :
              null
              }

            {allContent ?
            <div id={styles.FloatInfo}>
              <h3>Nombre : {element.name}</h3>
            {element.description!=null ?
              <p>Descripcion : {element.description}</p>
              :
              (null)}
              <p>ID : {element._id}</p>
              {element.members!=null?
              <p>Members : {element.members}</p>
              :null
              }
              {element.owner!=null?
              <p>Owner : {element.owner}</p>
              :
              null
              }

              {element.created!=null?
              <p>Created : {element.created.split('T')[0]}</p>
              :
              null
              }

            </div>
            :
            null
          }
      </button>
      <div id={styles.iconCont}>
        <Edit url={`/${url}/${element._id}`} updateState={updateState} nameElement={element.name} descriptionElement={element.description}></Edit>
        <DeleteButton url={`/${url}/${element._id}`} updateState={updateState}></DeleteButton>
      </div>
      </div>
    </div>
  )
}
